        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-gray-400 mx-1"></i>
                            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">{{ __('subscriptions.breadcrumbs.create') }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2 class="text-2xl font-bold mb-6 text-[#0061F5]">{{ __('subscriptions.breadcrumbs.create') }}</h2>
                    ...
                    <form action="{{ route('admin.suscripciones.store') }}" method="POST" enctype="multipart/form-data" x-data="{ 
                        ...
                        <!-- Usuario -->
                        <div class="mb-6">
                            <label for="user_id" class="block text-sm font-bold text-gray-700">{{ __('subscriptions.forms.doctor_label') }}</label>
                            <select name="user_id" id="user_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0061F5] focus:ring-[#0061F5]" required>
                                <option value="">{{ __('subscriptions.forms.select_doctor') }}</option>
                        ...
                        <!-- Selección de Item -->
                        <div class="mb-6">
                            <label for="item_id" class="block text-sm font-bold text-gray-700" x-text="tipo === 'paquete' ? '{{ __('subscriptions.forms.select_package') }}' : '{{ __('subscriptions.forms.select_item') }}'"></label>
                            ...
                            <select name="item_id" id="paquete_select" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0061F5] focus:ring-[#0061F5]" 
                                ...
                                <option value="">-- {{ __('subscriptions.forms.package') }} --</option>
                            ...
                            <select name="item_id" id="catalogo_select" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0061F5] focus:ring-[#0061F5]" 
                                ...
                                style="display: none;">
                                <option value="">-- {{ __('subscriptions.forms.select_item') }} --</option>
                            ...
                            <div class="mt-4" x-show="tipo === 'individual'" style="display: none;">
                                <label for="cantidad" class="block text-sm font-bold text-gray-700">{{ __('subscriptions.forms.quantity') }}</label>
                        ...
                        <!-- Método de Pago -->
                        <div class="mb-6">
                            <label class="block text-sm font-bold text-gray-700 mb-2">{{ __('subscriptions.forms.payment_method') }}</label>
                            <select name="metodo_pago" x-model="metodo_pago" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0061F5] focus:ring-[#0061F5]" required>
                                <option value="tarjeta">{{ __('subscriptions.forms.card') }}</option>
                                <option value="transferencia">{{ __('subscriptions.forms.transfer') }}</option>
                            </select>
                        </div>

                        <!-- Comprobante (Solo si es transferencia) -->
                        <div class="mb-6" x-show="metodo_pago === 'transferencia'" x-transition>
                            <label for="comprobante_pago" class="block text-sm font-bold text-gray-700">{{ __('subscriptions.forms.payment_voucher') }}</label>
                            ...
                            <p class="text-xs text-gray-500 mt-1">{{ __('subscriptions.forms.voucher_hint') }}</p>
                        </div>

                        <div class="flex justify-end mt-6">
                            <a href="{{ route('admin.suscripciones.index') }}" class="px-4 py-2 bg-gray-500 text-white font-bold rounded-md hover:bg-gray-600 transition-colors mr-2">{{ __('common.buttons.cancel') }}</a>
                            <button type="submit" class="px-4 py-2 bg-[#0061F5] text-white font-bold rounded-md hover:bg-[#0051CC] transition-colors">{{ __('common.buttons.save') }}</button>
                        </div>
                    </form>
                </div>
            </div>
