                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">{{ __('clinicas.title') }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-xl font-bold text-gray-800">{{ __('clinicas.title') }}</h2>
                    ...
                    <x-table>
                        ...
                        <x-slot:header>
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <th class="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">{{ __('common.address') }}</th>
                            ...
                            @forelse ($clinicas as $clinica)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $clinica->activo ? __('status.active') : __('status.inactive') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $clinica->creator->name ?? __('common.none') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end items-center space-x-2">
                                            <a href="{{ route('clinicas.edit', $clinica) }}" class="inline-flex items-center justify-center w-9 h-9 bg-[#0061F5] text-white rounded-md hover:bg-[#0051CC] transition-colors shadow-sm" title="{{ __('common.buttons.edit') }}">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('clinicas.destroy', $clinica) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ $confirmDeleteRecord }}');">
                                                @csrf
                                                @method('DELETE')
