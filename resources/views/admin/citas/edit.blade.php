        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <nav class="flex mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-gray-400 mx-1"></i>
                            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Editar Cita</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h2 class="text-2xl font-bold mb-6">Editar Cita</h2>
    ...
    <script>
        function appointmentForm() {
            return {
                ...
                clinicas: [],
                selectedClinicaId: '{{ $cita->clinica_id }}',
                ...
                selectedPatientId: '{{ $cita->paciente_id }}',
                
                slots: {},
                totalSlots: 0,
                selectedSlot: '{{ $cita->hora_inicio->format("H:i") }}',
                isLoading: false,
                ...
                async fetchSlotsIfReady() {
                    ...
                    try {
                        ...
                        const response = await fetch(`{{ route('api.slots') }}?doctor_id=${this.selectedDoctorId}&consultorio_id=${this.selectedConsultorioId}&fecha=${this.fecha}&except_cita_id={{ $cita->id }}`);
                        const data = await response.json();
                        
                        if (data.slots && (data.slots.Mañana || data.slots.Tarde || data.slots.Noche)) {
